"use client";

import { useInfiniteHodInstructors } from "../../hooks/use-instructors";
import { InfiniteScrollTrigger } from "@workspace/ui/components/infinite-scroll-trigger";
import { InstructorCard } from "./instructor-card";

export const HodInstructorList = () => {
    const { instructors, fetchNextPage, hasNextPage, isFetchingNextPage } =
        useInfiniteHodInstructors();

    return (
        <div className="space-y-4">
            {instructors.map((instructor) => (
                <InstructorCard
                    key={instructor.instructor.id}
                    instructorData={instructor}
                />
            ))}

            <InfiniteScrollTrigger
                canLoadMore={hasNextPage}
                isLoadingMore={isFetchingNextPage}
                onLoadMore={fetchNextPage}
                noMoreText="No more instructors to load"
            />
        </div>
    );
};

export const HodInstructorListSkeleton = () => {
    return (
        <div className="space-y-4">
            {Array.from({ length: 5 }).map((_, index) => (
                <div
                    key={index}
                    className="p-4 border rounded-md shadow-sm bg-gray-100 animate-pulse"
                >
                    <div className="h-4 bg-gray-300 rounded w-1/4 mb-2"></div>
                    <div className="h-6 bg-gray-300 rounded w-3/4"></div>
                    <div className="h-4 bg-gray-300 rounded w-1/2 mt-2"></div>
                    <div className="h-4 bg-gray-300 rounded w-1/3 mt-2"></div>
                </div>
            ))}
        </div>
    );
};
