...
import { useSuspenseInstructor } from "@/modules/tenant/hooks/use-instructor";
import { Skeleton } from "@workspace/ui/components/skeleton";

interface InstructorActionsCardProps {
    id: string;
}

export const InstructorActionsCard = ({ id }: InstructorActionsCardProps) => {
    const { data } = useSuspenseInstructor(id);
    const { instructor, user, department } = data;
...
export const InstructorActionsCardSkeleton = () => {
    return (
        <Card className="p-6 space-y-4">
            <div className="flex items-center justify-between">
